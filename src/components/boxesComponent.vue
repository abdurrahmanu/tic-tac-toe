<template>
        <div  class="relative z-30 pt-4 md:w-full md:h-full md:grid md:place-content-center">
            <div ref="boxesContainer" :class="{'gap-3': gameStore.isDarkMode}"  class="grid grid-cols-3 grid-rows-3 gap-1 md:w-[400px] md:h-[400px] w-[300px] min-w-[150px] h-[300px] justify-center m-auto ">
                ...
                v-for="index in 9"
                :key="index"
                :windIndex="winnerCombination.length && winnerCombination.includes(index)"
                :index="index" />
            </div>
...
const {playHasStarted, playerOneWins, boxes, winnerCombination, playerTwoWins, drawTies, computerChoiceIndex, computerAlphabet, vsPC} = storeToRefs(gameStore)
